...
class OLTC_controller(Controller, records.DCTL):
    prefix = "DCTL LTC2"

    def __init__(self, OLTC, delay_1, delay_2, dir):
        """
        All other parameters (static ones) are read from the OLTC.
        """

        self.name = OLTC.trafo.name
        self.OLTC = OLTC
        self.delay_1 = delay_1
        self.delay_2 = delay_2
        self.dir = dir
        self.cumulative_dr = 0
        self.delay_RAMSES_1 = delay_1
        self.delay_RAMSES_2 = delay_2
        self.initial_half_db_RAMSES = 2
        self.v_setpoint = self.OLTC.v_setpoint_pu
        self.half_db = self.initial_half_db_RAMSES
    ...
    def increment_r(self, dr):
        """
        Add dr to the count of cumulative dr.
        """

        self.cumulative_dr += dr

    def make_overridable(self):
        self.is_overridable = True

    def get_disturbances(self, t, new_v_setpoint, new_half_db):
        delta_v_setpoint = new_v_setpoint - self.v_setpoint
        self.v_setpoint += delta_v_setpoint
        delta_half_db = new_half_db - self.half_db
        self.half_db += delta_half_db

        return [
            sim_interaction.Disturbance(t, self, "Vsetpt", delta_v_setpoint),
            sim_interaction.Disturbance(t, self, "DB", delta_half_db),
        ]

    def increase_voltages(self, t):
        """
        Increase the voltages at time t.
        """

        self.OLTC.increase_voltages()

        return self.get_disturbances(
            t, new_v_setpoint=2, new_half_db=0
        ) + self.get_disturbances(
            t + 1.5 * self.delay_RAMSES_1, new_v_setpoint=1, new_half_db=2
        )

    def reduce_voltages(self, t):
        """
        Decrease the voltages at time t.
        """

        self.OLTC.reduce_voltages()

        return self.get_disturbances(
            t, new_v_setpoint=0, new_half_db=0
        ) + self.get_disturbances(
            t + 1.5 * self.delay_RAMSES_1, new_v_setpoint=1, new_half_db=2
        )

    def try_to_move_OLTC(self):
        """
        Try to move the OLTC.

        The only thing that could stop it is that the cumulative dr is not
        enough.
        """
        ...
        # Time between t_now and the time when the disturbance takes place
    ...
    def v_is_below(self):
        return self.measure_v() < self.OLTC.v_setpoint - self.OLTC.half_db

    def v_is_above(self):
        return self.measure_v() > self.OLTC.v_setpoint + self.OLTC.half_db

    def v_is_within(self):
        return not self.v_is_above() and not self.v_is_below()

    def get_actions(self, t_now):
        """
        Return disturbances based on FSM.
        """

        elapsed = t_now - self.t_last_action
        dists = []

        # Make transitions based on current measurements
    ...
    def get_pars(self):
        return [
            ...
            records.Parameter("tol", self.initial_half_db_RAMSES),
